				...
				if ((string)$cost != '') {
				 ...
				 $costnew = $cost + $p2;
				    ...
				    if ($this->config->get('b_dhl_geo_weight_shipdisplayoption') =='1') { 
						...
						'text'         => $this->currency->format($this->tax->calculate($costnew, $this->config->get('b_dhl_geo_weight_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
					    );
